...
const Auth = () => {
  ...
  const detectedLocation = loc.state?.location || { city: "", country: "", continent: "" };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] border border-border/20 rounded-full animate-spin-slow" />
      </div>
      ...
      >
        ...
        <div className="flex items-center bg-muted rounded-full p-1 mb-6">
          {(["consumer", "enterprise"] as UserType[]).map((type) => (
            <button
              key={type}
              onClick={() => setUserType(type)}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-full text-sm font-display font-semibold transition-all ${
                userType === type
                  ? "bg-foreground text-background shadow-md"
                  : "text-muted-foreground hover:text-foreground"
              }`}
            >
              {type === "consumer" ? <User className="w-4 h-4" /> : <Building2 className="w-4 h-4" />}
            ...
            >
              {tab === "signup" && (
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder={userType === "enterprise" ? "Full Name" : "Your Name"}
                    value={form.name}
              ...
              {tab === "signup" && userType === "enterprise" && (
                <div className="relative">
